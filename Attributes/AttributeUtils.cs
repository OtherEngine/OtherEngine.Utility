...
namespace OtherEngine.Utility.Attributes
{
	public static class AttributeUtils
	{
		...
		/// <summary> Returns all the attribute provider's attributes of type TAttr.
		///           If inherit is true, checks inherited attributes. </summary>
		/// <exception cref="AttributeInstanciationException">
		///   Thrown when an exception was encountered while attempting to get the attributes.
		///   Likely caused by exceptions thrown in the attribute's constructor. </exception>
		public static IEnumerable<TAttr> GetAttributes<TAttr>(
			this ICustomAttributeProvider target, bool inherit = false)
			where TAttr : Attribute
		{
			if (target == null)
				throw new ArgumentNullException("target");

			try { return target.GetCustomAttributes(typeof(TAttr), inherit).Select(a => (TAttr)a); }
			catch (Exception ex) { throw new AttributeInstanciationException(typeof(TAttr), target, ex); }
		}

		/// <summary> Returns all the type's attributes of type TAttr,
		///           If inherit is true, checks inherited attributes. </summary>
		...
		/// <summary> Returns the attribute provider's attribute of type TAttr,
		...
		public static TAttr GetAttribute<TAttr>(
			...
			where TAttr : Attribute
		{
			return target.GetAttributes<TAttr>(inherit).FirstOrDefault();
		}

		/// <summary> Returns the type's attribute of type TAttr, null if
		...
		/// <summary> Enumerates all members of the current type to get members of type
		///           TMember which have one or multiple attributes of type TAttr. </summary>
			...
			where TMember : MemberInfo where TAttr : Attribute
		{
			return type.GetMembers(bindingAttr).OfType<TMember>()
				.SelectMany(m => m.GetAttributes<TAttr>()
		...
		/// <summary> Gets the name of an attribute target.
		///           For example: [Assembly: OtherEngine.Core],
		///                        [Type: TestController],
		///                        [Method: TestController.OnEvent],
		///                        [Property: TestController.Entities],
		///                        [Parameter: TestController.DoSomething input],
		///                        [Return: TestController.DoSomething] </summary>
